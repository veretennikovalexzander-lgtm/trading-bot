...
def _validate_config():
    ...
    if cfg.binance.api_key.lower() in dangerous or not cfg.binance.api_key:
        logger.error("SECURITY: API key not set. Edit .env")
        sys.exit(1)
    if cfg.binance.api_secret.lower() in dangerous or not cfg.binance.api_secret:
        logger.error("SECURITY: API secret not set. Edit .env")
        sys.exit(1)
    logger.info("API keys validated")


def run_bot():
    """Start the trading bot: controller + watchdog + WebSocket streamer."""
    setup_logging()
    _validate_config()
    init_db()
    ...
    logger.info("=== Bot STARTED (WebSocket) ===")
